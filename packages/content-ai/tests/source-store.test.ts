...
import { aiEventSchema, sourceDescriptorSchema } from "../src/schemas/ai-events.ts";
import { normalizeSourceField } from "../src/schemas/ai-events.ts";

describe("aiEvents.ingested.source — schema migration + back-compat", () => {
  const base = {
    type: "ingested" as const,
    suggestion: { hash: "abc123", summary: "Pasta" },
    at: "2026-05-08T00:00:00Z",
    model: "recipe-ingestion",
  };

  test("old string URL source still validates", () => {
  ...
  test("new structured source descriptor validates", () => {
    const result = aiEventSchema.safeParse({
      ...base,
      source: {
        kind: "pdf",
        ...
        mime: "application/pdf",
        sizeBytes: 512,
        ingestedAt: "2026-05-08T12:00:00Z",
      },
    });
    expect(result.success).toBe(true);
  });

  test("structured source with optional url and filename validates", () => {
    const result = aiEventSchema.safeParse({
      ...base,
      source: {
        kind: "url",
  ...
  test("sourceDescriptorSchema rejects missing required fields", () => {
    const result = sourceDescriptorSchema.safeParse({ kind: "pdf" });
    expect(result.success).toBe(false);
  });

  test("normalizeSourceField converts string to { kind: 'url', url: string } descriptor", () => {
    const result = normalizeSourceField("https://example.com/recipe");
  ...
  test("normalizeSourceField passes through structured descriptor unchanged", () => {
    const descriptor = {
      kind: "pdf" as const,
      hash: "a".repeat(64),
      mime: "application/pdf",
    ...
    const result = normalizeSourceField(descriptor);
    expect(result).toEqual(descriptor);
  });

  test("normalizeSourceField returns undefined for undefined input", () => {
