...
import { generateTraceId } from "./ids.ts";
import type { TraceSink, TraceEvent, TraceFinishReason } from "./sinks/types.ts";
...
async function fan(sinks: TraceSink[], event: TraceEvent): Promise<void> {
  await Promise.allSettled(sinks.map((s) => s.emit(event)));
}
