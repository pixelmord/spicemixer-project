...
import {
  ...
  type FieldDiff,
  type ChangeKind,
} from "@/lib/ingredient-diff.ts";

// ── Shared helpers (mirrors RecipeDiff structure) ────────────────────────────
...
export default function IngredientDiff({ existing, proposed }: Props) {
  const diffs = diffIngredients(existing, proposed);
  const changed = diffs.filter((d) => d.kind !== "unchanged");

  if (!hasIngredientChanges(diffs)) {
    return (
      <div className="rounded-lg border border-border bg-muted/30 p-6 text-center text-sm text-muted-foreground">
        No differences detected between the existing and proposed ingredient.
      </div>
    );
  }
